...
class Carousel(tk.Frame):
        ...
        for i, item in enumerate(self.items):
            try:
                with Image.open(item['image_path']) as img:
                    # --- FIX: Adjust resizing for theme cards ---
                    # Maintain aspect ratio for the main card
                    large_img = img.copy()
                    large_img.thumbnail((240, 128), Image.Resampling.LANCZOS)
                    
                    # Smaller side cards
                    small_img = img.copy()
                    small_img.thumbnail((100, 75), Image.Resampling.LANCZOS)
    ...
    def update_display(self):
        ...
        center_y = self.winfo_height() / 2 if self.winfo_height() > 1 else 90

        # rendering order matters, drawn first is at the back
        # Previous Item
        ...
        if prev_item_images:
            ...
            if prev_photo:
                self.canvas.create_image(50, center_y, image=prev_photo, tags="prev")
        ...
        if next_item_images:
            ...
            if next_photo:
                self.canvas.create_image(self.config.SCREEN_WIDTH - 50, center_y, image=next_photo, tags="next")
